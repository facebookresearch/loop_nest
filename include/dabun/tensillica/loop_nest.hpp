...
class loop_nest_code_generator
    : public cpp_intrinsics_code_generator<void(float* C, float const* A,
                                                float const* B, int alpha)>
{
private:
...
private:
    ...
    static constexpr int default_max_operations_unrolled = 16; // 320;
    static constexpr int isa_available_vector_registers  = 32;

    using multi_vregs = multi_vmm<Vmm, SReg, Reg32>;
...
private:
        ...
        std::function<void()> const& epilogue_fn)
    {
            ...
            std::visit(
                overloaded{
                    [&](load_pair_instruction const& i)
                    {
                        ...
                        {
                            ins(VReg(i.vreg1).s[1], ZeroReg32_);
                            ins(VReg(i.vreg2).s[1], ZeroReg32_);
                        }
                    ...
                    [&](load_instruction const& i)
                    {
                        ...
                        {
                        case 1:
                            meta_ldr_post_ptr(SReg(i.vreg), Reg64(ptr_reg_idx),
                                              delta);
                            if (C_traits.access == SCALAR)
                            {
                                ins(VReg(i.vreg).s[1], ZeroReg32_);
                            }

                            break;
                        case 2:
                            meta_ldr_post_ptr(DReg(i.vreg), Reg64(ptr_reg_idx),
                                              delta);
                            break;
                        case 3:
                            meta_ldr_post_ptr(QReg(i.vreg), Reg64(ptr_reg_idx),
                                              delta);
                            if (C_traits.access == SCALAR)
                            {
                                ins(VReg(i.vreg).s[3], ZeroReg32_);
                            }
                            break;
                        case 4:
                            meta_ldr_post_ptr(QReg(i.vreg), Reg64(ptr_reg_idx),
        ...
        {
            meta_sadd_imm(Reg64(offs.first), -offs.second);
        }
            ...
            std::visit(
                overloaded{
                    ...
                    [&](load_wreg_instruction const& i)
                    {
                        ...
                        meta_ldr_post_ptr(Reg32(i.reg), Reg64(ptr_reg_idx),
                                          delta);
                    },
                    [&](load_xreg_instruction const& i)
                    {
                        ...
                        meta_ldr_post_ptr(Reg64(i.reg), Reg64(ptr_reg_idx),
                                          delta);
                    },
                    [&](ins_wreg_instruction const&) { strong_assert(false); },
                    [&](ins_xreg_instruction const&) { strong_assert(false); },
    ...
    void set_available_vector_registers()
    {
        ...
        available_registers =
            isa_available_vector_registers - auxiliary_registers;
...
private:
    ...
    std::set<memory_argument>
    collect_default_loads_and_stores_at([[maybe_unused]] int depth)
    {
        std::set<memory_argument> ret;
                             ...
                             int innermost_operations)
    {
        ...
        // TODO(zi) better heuristics here
        {
            ...
            strong_assert(next <= isa_available_vector_registers);
        ...
        std::vector<operation_operation> operations, int num_iterations)
    {
        ...
        auto num_regs =
            isa_available_vector_registers - first_unused_vmm_register;
...
private:
    void issue_C_loads(std::set<memory_argument> const& loads)
    {
        ...
        for (auto const& c : ordered_loads)
        {
            ...
            {
                mov(C_VMMs[c][s].s4, ZeroVector_.s4);
            }
        }
    }
                        ...
                        bool /* issue_max_alpha_logic */)
    {
        ...
        if (false)
        {
            ...
            b(comparison::LT, *donePostOpLabel);
            ...
            for (auto const& c : ordered_stores)
            {
                ...
                else
                {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        fmax(C_VMMs[c][0].s2, C_VMMs[c][0].s2, ZeroVector_.s2);
                        break;
                    case 3: // fall through
                    case 4:
                        fmax(C_VMMs[c][0].s4, C_VMMs[c][0].s4, ZeroVector_.s4);
                        break;
                    default:
...
public:
    ...
    loop_nest_code_generator(
        [[maybe_unused]] std::vector<std::pair<std::string, int>> const& _order,
        [[maybe_unused]] std::map<std::string, int> const&               sizes,
        [[maybe_unused]] std::set<std::string> const&      C_formula,
        [[maybe_unused]] std::set<std::string> const&      A_formula,
        [[maybe_unused]] std::set<std::string> const&      B_formula,
        [[maybe_unused]] std::map<std::string, int> const& C_strides,
        [[maybe_unused]] std::map<std::string, int> const& A_strides,
        [[maybe_unused]] std::map<std::string, int> const& B_strides,
        std::shared_ptr<void> /*op_pair */,
    ...
    //       elementwise_postop_strides, optim_config)
    {
        ...
        [[maybe_unused]] int first_loop_that_can_hold_C, unroll_stage,
            total_required_innermost_operations;
        ...
        [[maybe_unused]] int depth_for_register_blocked_C =
            first_loop_that_can_hold_C;
        [[maybe_unused]] int innermost_operations =
            total_required_innermost_operations;
        ...
        prepare_stack();
        eor(ZeroReg32_, ZeroReg32_, ZeroReg32_);
        ins(ZeroVector_.s[0], ZeroReg32_);
        ins(ZeroVector_.s[1], ZeroReg32_);
        ins(ZeroVector_.s[2], ZeroReg32_);
        ins(ZeroVector_.s[3], ZeroReg32_);
