...
auto loop_nest_slow_baseline(
    ...
    std::map<std::string, int> const& B_strides, bool perform_relu = false)
{
    return [=](float* Cptr, float const* Aptr, float const* Bptr, int alpha) {
        ...
        std::function<void(float*, float const*, float const*, int)>
            recursive_compute = [&](float* C, float const* A, float const* B,
                                    int order_depth) {
                if (order_depth == order.size())
                {
                    C[0] += A[0] * B[0];
                ...
                else
                {
                    ...
                    for (int i = 0; i < full; ++i)
                    {
                        ...
                        C += C_strides.count(var) ? C_strides.at(var) * delta
                                                  : 0;
                        A += A_strides.count(var) ? A_strides.at(var) * delta
                                                  : 0;
                        B += B_strides.count(var) ? B_strides.at(var) * delta
                                                  : 0;
                    }
                    limits[var].pop_back();
...
auto loop_nest_baseline(std::vector<std::pair<std::string, int>> const& order,
                        std::map<std::string, int> const&               sizes,
                        std::set<std::string> const& C_formula,
                        std::set<std::string> const& /*A_formula*/,
                        std::set<std::string> const& /*B_formula*/,
