...
int main() {
  ...
  // a single element and 0 strides.
  {
    ...
    auto secs = measureFastestWithWarmup(
        [&]() { fn.run_on_aligned_data(CJ.data(), &A, B.data()); }, 10, 100);
  ...
  // C(r, c) = A(r, k) * B(k, c)
  {
    ...
    auto fn = facebook::sysml::aot::LoopNestToHalide<DABUN_ISA>(
        ...
        {{"ArCr", 123},
         ...
         {"AcBr", 1}, // inner loops, should handle
                      // differently later
         {"ArCr", 1},
         {"BcCc", 112}, // TODO DEBUG NOT INJECTING THE
         {"BcCc", 1}},
        // The second argument is a map of the dimension sizes
        {{"AcBr", AcBr}, {"ArCr", ArCr}, {"BcCc", BcCc}},
        // Vars of C (other variables are reduction variables)
    ...
    baseline_MM(ArCr, AcBr, BcCc, AcBr, 1, BcCc, 1, BcCc, 1, A.data(), B.data(),
                CN.data(), 0);
  ...
  // C(r, c) = A(r, k) * B(k, c)
  {
    ...
    auto CN = get_random_vector<float>(ArCr * BcCc + 16);
    auto CJ = CN;
  ...
  // (row) Vector - (row - major) Matrix product C(c) = A(k) * B(k, c) if (0)
  {
  ...
  // k_h, k_w)
  {
    ...
    auto CN = get_random_vector<float>(COUT * OS * OS);
    auto CJ = CN;
