...
CASES = {
    ...
    "reg-block + reg-split": (False, True, False),
    "reg-block + reg-split + issue-delay": (True, True, False),
    "reg-block + reg-split + issue-delay + addr-pack (full)":
    (True, True, True),
}


def get_optimization_config(case):
    code = "OptimizationConfiguration({}, {}, {})"
    bools = (str(b).lower() for b in CASES[case])
    return code.format(*bools)


def run_loop_nest(loop_nest_root, arch, case, input_path, output_path):
    cmd = [
        "g++",
        "-Wall",
        "-Wpedantic",
...
def dump_summary(summary_log, output_path):
    ...
    with open(output_path, "w") as fout:
        ...
        writer.writerow(
            ["system", "isa", "case", "benchmark", "compile", "gflops"])
    ...
    for arch in archs:
        for case in CASES.keys():
            output_path = os.path.join(output_dir,
                                       "ablation-{}-{}.txt".format(arch, case))
            print("Running loop_nest on", output_path)
            ret = run_loop_nest(loop_nest_root, arch, case, cpp_file,
                                output_path)
            if ret != 0:
                print("loop_nest failed on", output_path)
            results = extract_results(output_path)
            results = [tuple([arch, case] + entry) for entry in results]
